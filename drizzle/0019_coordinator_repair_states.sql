CREATE TABLE IF NOT EXISTS "coordinator_repair_states" (
  ...
  CONSTRAINT "coordinator_repair_states_owner_scope_length" CHECK (
    char_length("owner_scope_id") BETWEEN 16 AND 256
  ),
  CONSTRAINT "coordinator_repair_states_execution_scope_length" CHECK (
    char_length("execution_scope_id") BETWEEN 16 AND 256
  ),
  CONSTRAINT "coordinator_repair_states_revision_range" CHECK (
    "revision" BETWEEN 0 AND 9007199254740991
  ),
  CONSTRAINT "coordinator_repair_states_envelope_object" CHECK (
    jsonb_typeof("envelope") = 'object'
  ),
  CONSTRAINT "coordinator_repair_states_envelope_version" CHECK (
    "envelope" ->> 'version' = '1'
  ),
  CONSTRAINT "coordinator_repair_states_envelope_owner_scope" CHECK (
    "envelope" ->> 'ownerScopeId' = "owner_scope_id"
  ),
  CONSTRAINT "coordinator_repair_states_envelope_execution_scope" CHECK (
    "envelope" ->> 'executionScopeId' = "execution_scope_id"
  ),
  CONSTRAINT "coordinator_repair_states_envelope_snapshot" CHECK (
    jsonb_typeof("envelope" -> 'snapshot') = 'object'
  ),
...
CREATE INDEX IF NOT EXISTS "coordinator_repair_states_updated_at_idx"
  ON "coordinator_repair_states" ("updated_at");
