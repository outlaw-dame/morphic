...
export function evaluateEntityGrounding(
  ...
  if (!state.routePlan.needsEntityGrounding) {
    return passPolicy(
      'entity_grounding',
      'Route does not require entity grounding.'
    )
  }
  ...
  if (groundedItems.length === 0) {
    return failPolicy({
      ...
      severity: 'block',
      reason:
        'Route requires entity grounding but usable evidence has no resolved entities.',
  ...
  if (ambiguousEntities.length > 0) {
    return failPolicy({
      ...
      severity: 'warn',
      reason:
        'Some grounded entities are ambiguous and should be disambiguated before final composition.',
