...
import { SourceClassSchema, type SourceClass } from '@/lib/ai/schemas'
import {
  runRole,
...
const MAX_RESULTS_PER_PATH = 50

export const FusionPathPurposeSchema = z.enum([
  'primary_evidence',
...
export type FusionPathPurpose = z.infer<typeof FusionPathPurposeSchema>
  ...
  .transform(value =>
    Object.freeze({
      ...
      disallowedSourceClasses: Object.freeze([
        ...value.disallowedSourceClasses
      ])
    })
  )

export const FusionPathSchema = z
  .object({
    id: z.string().regex(/^[a-z0-9][a-z0-9_-]{0,63}$/),
    query: z.string().trim().min(1).max(MAX_PATH_QUERY_LENGTH),
    sourceClass: SourceClassSchema,
    purpose: FusionPathPurposeSchema,
    maxResults: z.number().int().min(1).max(MAX_RESULTS_PER_PATH),
...
const FusionPlannerOutputSchema = z
  ...
  .superRefine((value, context) => {
    ...
    const queries = value.paths.map(path => normalizeSemanticQuery(path.query))
    if (new Set(queries).size !== queries.length) {
      context.addIssue({
        code: 'custom',
        message: 'Fusion path queries must be semantically distinct.',
        path: ['paths']
      })
    }
  })
...
export type ProductionFusionPath = z.infer<typeof FusionPathSchema>
...
export type ProductionFusionPlanner = Readonly<{
  plan(input: Readonly<{
    query: string
...
const COMMUNITY_SOURCE_CLASSES = new Set<SourceClass>([
  'forum_or_reddit',
  'social_media'
])
...
function requiredDiversity(mode: FusionPlannerModelInput['mode']): number {
  switch (mode) {
    case 'quick':
      return 1
    case 'adaptive':
...
function validatePlan(
...
): readonly ProductionFusionPath[] {
  if (output.paths.length > input.maxToolCalls) {
    throw new Error('Fusion Planner exceeded the route tool-call budget.')
  }
  ...
  if (
    ...
    !output.paths.some(
      path => path.requiresFreshness && path.purpose === 'freshness_check'
    )
  ) {
  ...
  if (
    ...
    !output.paths.some(path => path.purpose === 'entity_disambiguation')
  ) {
    throw new Error('Fusion Planner omitted the required entity path.')
  }

  const distinctClasses = new Set(output.paths.map(path => path.sourceClass))
  const minimumDiversity = Math.min(
    requiredDiversity(input.mode),
    input.maxToolCalls
  )
  if (distinctClasses.size < minimumDiversity) {
    throw new Error('Fusion Planner did not satisfy minimum source diversity.')
  }

  const communityPaths = output.paths.filter(path =>
    COMMUNITY_SOURCE_CLASSES.has(path.sourceClass)
  )
  if (
    communityPaths.length > 1 ||
    communityPaths.some(path => path.maxResults > 5)
  ) {
    throw new Error('Fusion Planner exceeded the community-source influence cap.')
  }

  const aggregateResults = output.paths.reduce(
    (total, path) => total + path.maxResults,
    0
  )
  if (aggregateResults > Math.min(500, input.maxToolCalls * 50)) {
    throw new Error('Fusion Planner exceeded the aggregate result budget.')
  }
