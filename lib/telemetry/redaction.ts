 ...
 *
 * Rules:
 * - Search queries are redacted (replaced with [QUERY])
 * - Auth tokens / API keys are redacted
 * - User email / PII is redacted
 * - File paths with user directories are redacted
 * - URL query params with sensitive names are redacted
 * - Route paths are classified, not sent raw
 */
...
const REDACTION_PATTERNS: Array<{ pattern: RegExp; replacement: string }> = [
  ...
  { pattern: /key-[a-zA-Z0-9]{10,}/g, replacement: '[REDACTED_KEY]' },
  // Bearer tokens
 ...
 * This prevents user-specific data from appearing in route-level metrics.
 */
 ...
 */
export function buildCrashMetadata(options: {
  appVersion?: string
  platform?: string
  runtimeKind?: string
  routeClass?: string
